...
from src.SymbolicMath import CosineSurface
...
def modelRun():
    ...
    # Number of waves to recover
    cosine_count = 1

    # Microphone array Locations
    ...
    #Angle of the source relative to the surface
    SourceAngle = np.pi / 3

    #Experimental Microphone Data - LVV 2019 data at 14kHz
    truescatter = [0.01260586, 0.03210286, 0.06278351, 0.05601213, 0.03996005, 0.02276676,
    ...
    # True surface parameters
    p = (0.0015, 0.05, 0.0)

    # Number of samples for Kichhoff integral
    userSamples = 700
    ...
    # Load the results data. Amp and Wl are in units of m, phase is in units of rad/2*pi.
    posterior_samples_grouped = np.array(AcousticParameterMCMC.LoadCSVData("results/" + kernel + ".csv"))

    # Normalize true scatter here to compare to parameters
    ...
    plt.fill_between(x,mins,maxx,color='#ffffbf',label=r'Credible interval (68\%)')
    ...
    # Pick 300 samples from the trace
    choice_count = 300
    ...
    # Generate the corner plot. Scaled to cm and radians.
    ...
    # Generate the relative error plot
    plt.figure(figsize=(16,6))
    plt.grid(which='both')
    plt.grid(which='minor', alpha=0.4)
    plt.grid(which='major', alpha=0.75)
    plt.grid()
    plt.plot(x,np.sqrt((mean-true)**2)/np.std(true),color='#2c7bb6',linestyle='dashed',label=r"Relative error of the mean of the surfaces")
    plt.plot(x,np.sqrt((mean_surf-true)**2)/np.std(true),color='#2c7bb6',label=r"Relative error of the mean of the parameters")
    plt.xlabel("x (m)")
