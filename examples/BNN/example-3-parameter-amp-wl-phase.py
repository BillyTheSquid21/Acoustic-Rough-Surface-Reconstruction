...
if __name__ == "__main__":
    ...
    bnn = BayesianNN(nHidden=indices[-1]*1.5)
    bnn.setName("amp_wl_phase_bnn")
    # Set the penalty mask so phase is allowed to be negative
    bnn.train(X3_train, Y3_train, sampleCount=20_000, burnInCount=5000, penaltyMask=pt.as_tensor_variable((1.0,1.0,0.0)))
    predict = bnn.predict(X3_test, Y3_test, penaltyMask=pt.as_tensor_variable((1.0,1.0,0.0)))
    ...
    pred_phase_sorted = np.apply_along_axis(AngularMean, arr=phases_sorted.copy()/(2.0*np.pi), axis=0)*2.0*np.pi
    ...
    def circular_r2_score(true, pred):
        ...
        den = (np.arccos(np.cos(true-AngularMean(true)))**2).sum()
        return 1.0 - ((num)/(den))
