...
if __name__ == "__main__":
    ...
    def generate_microphone_pressure(parameters,uSamples=700):
        def newFunction(x):
            return CosineSurface(x,parameters).copy()
    ...
    if generate_data:
        ...
        for i in range(p_count):
            params.append((0.0015, wlspace[i], 0.0))
            
        params = np.array(params)
        np.savetxt("results/wl_only_params_14KHz.csv", params, delimiter=",")
        ...
        plt.legend()
        plt.show()
        np.savetxt("results/wl_only_scatter_14KHz.csv", scatters, delimiter=",")

    params = np.loadtxt("results/wl_only_params_14KHz.csv", delimiter=",")
    scatters = np.loadtxt("results/wl_only_scatter_14KHz.csv", delimiter=",")
    training_data = np.column_stack((params[:,1:2], scatters))
    ...
    lr = LinearRegression()
    lr.fit(X3_train, Y3_train['wl'])
    print("Linear score: ", lr.score(X3_test, Y3_test['wl']))
    plt.scatter(Y3_train['wl'], Y3_train['wl'], label="True values")
    plt.scatter(Y3_train['wl'], lr.predict(X3_train), color="red", label="Model predictions")
    plt.legend()
    plt.show()

    # Initialize and train model
    bnn = BayesianNN(nHidden=40)
    bnn.setName("wl_only_bnn")
    bnn.train(X3_train, Y3_train, sampleCount=5000, burnInCount=2000)
    wls = np.array(bnn.predict(X3_test, Y3_test)['param'])

    wl_index = np.array(wls).squeeze()
    pred_wl = wl_index.mean(axis=0)
